...
function DoctorReplies() {
    ...
    useEffect(() => {
        app.database().ref('list/'+Id).update({
            message : "end"
          })
          app.database().ref("message/"+Id).on("value", snapshot => {
            let memberlist = [];
    ...
    const handleSend=()=>{
        if(value!=""){
            app.database().ref('message/'+Id).push().update({
